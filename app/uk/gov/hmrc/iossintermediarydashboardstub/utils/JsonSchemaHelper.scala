...
import uk.gov.hmrc.iossintermediarydashboardstub.models.responses.{CoreErrorResponse, EisErrorResponse}
...
class JsonSchemaHelper extends Logging {
  ...
  def applySchemaHeaderValidation(headers: Headers)(f: => Future[Result]): Future[Result] = {
    ...
    HeaderHelper.validateHeaders(headers.headers) match {
      case Right(_) => f
      case Left(MissingHeader(header)) =>
        BadRequest(Json.toJson(EisErrorResponse(CoreErrorResponse(Instant.now(), Some(UUID.randomUUID()), "OSS_003", s"Bad Request - missing $header")))).toFuture
      case Left(InvalidHeader(header)) =>
        BadRequest(Json.toJson(EisErrorResponse(CoreErrorResponse(Instant.now(), Some(UUID.randomUUID()), "OSS_002", s"Bad Request - invalid $header")))).toFuture
      case _ =>
        BadRequest(Json.toJson(EisErrorResponse(CoreErrorResponse(Instant.now(), Some(UUID.randomUUID()), "OSS_000", s"Bad Request - unknown error")))).toFuture
    }
  }
